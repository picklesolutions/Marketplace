...
import "./CryptoDoggosInterface.sol";


/**
 * @title A small marketplace allowing users to buy and sell a specific ERC721 token
 * @dev For this example, we are using a fictional token called CryptoDoggos
...
contract Marketplace is Ownable {
  ...
  address public cryptoDoggosContractAddress = address(0);
  ...
  function createOffer(
    ...
    CryptoDoggosInterface cryptoDoggos = CryptoDoggosInterface(cryptoDoggosContractAddress);

    /* We need to be sure that the sender is the actual owner of the token */
    require(
      cryptoDoggos.ownerOf(itemId) == msg.sender,
      "Sender does not own this item"
    );

    /* Are we approved to manage this item? */
    require(
      cryptoDoggos.getApproved(itemId) == address(this),
  ...
  function buyItem(
    ...
    require(
      cryptoDoggos.getApproved(offers[offerId].itemId) == address(this),
      "We do not have approval to manage this item"
    );
   ...
   * @param newCryptoDoggosContractAddress The address of the contract
   */
  function setCryptoDoggosContractAddress(
    address newCryptoDoggosContractAddress
  ) external onlyOwner() {
    /* The address cannot be null */
    require(newCryptoDoggosContractAddress != address(0), "Contract address cannot be null");

    /* We set the address */
    cryptoDoggosContractAddress = newCryptoDoggosContractAddress;
  }
  ...
  function openMarketplace() external onlyOwner() {
    require(cryptoDoggosContractAddress != address(0), "Contract address is not set");
